...
\label{sec:Design}
In this section, we discuss SystemL's design of lineage capturing and instrumentation interface, 
storage space reduction and query performance optimization with indexing strategies.
...
SystemL exposes the lineage capturing interface by naming the lineage types with its mapping types between input and output datasets.
Table~\ref{tb:lineage-interface} summarizes the lineage types and parameters passing by the users.
\begin{table*}[ht]
\begin{center}
    \caption{Primitive Lineage Types and the Interface}
    \begin{scriptsize}
    \begin{tabular}{ | p{16cm}|}
    \hline
    CollapseLineage(in: RDD[A], out: RDD[B], dimension: Int)  \\ \hline 
    ExpansionLineage(in:RDD[A], out:RDD[B], dimension: Int) \\ \hline
    IdentityLineage(in:RDD[A], out:RDD[A]) \\ \hline
    AllLineage(in:RDD[A], out:RDD[A]) \\ \hline
    GeoLineage(in:RDD[A], out:RDD[A], mapping: RDD[List[(F,F)]]) \\ \hline
    LinComLineage(in:RDD[A], out:RDD[A], model: B) \\ \hline
    JoinLineage(in:Seq[RDD[A]], out:RDD[Seq[A]], dimention: Int) \\ \hline
    CustomLineage(in:A, out:B, fMap: C=>D, bMap: D=>C) \\ \hline
    \end{tabular}
    \end{scriptsize}
    \label{lst:lineage-interface}
\end{center}   
\end{table*}
