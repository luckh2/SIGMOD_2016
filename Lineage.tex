...
We can describe the subset as a list of random regions (squares in column sampling of a matrix), with each input region maps to an output region.
...
can be slow due to the encoding. In this section, we discuss three indexing strategies to speedup the query over the
region mapping. 
For simplicity, we only discuss forward query from the input to the output in this section.
The backward query has identical properties and the forward query.
...
\begin{center}
    ...
    (0,0),...,(1,1) & Circle\_0 & (0,0),(1,0) & Square\_0 \\ \hline
    (0,2),...,(1,3) & Circle\_1 & (0,1),(1,1) & Square\_1 \\ \hline
    (2,0),...,(3,1) & Circle\_2 & (0,2),(1,2) & Square\_2 \\ \hline
    (2,2),...,(3,3) & Circle\_3 & (0,3),(1,3) & Square\_3 \\ \hline
    \end{tabular}
    \end{scriptsize}
    \label{tb:example}
\end{center}   
...
We refer the non-index strategy as {\bf NoIndex}. 
With this strategy, query an element needs to test if the key is within a input region for all input regions.
If an element is in an input region, the query returns the output region with all output elements expanded.
The {\bf NoIndex} strategy does not build any index, thus the space complexity is 0.
The time complexity of a query is $O(k*N)$, where k is number of input regions and N is the number
of elements in each region. For simplicity, the time complexity of a query with {\bf NoIndex} is $O(N^2)$.

\subsubsection{Direct Index}
The direct index strategy is referred as {\bf Direct} in the future discussion.
To build the direct index, we use each input element as key and the associated region as the value.
We rely on the underlying system for the optimization of storing repeated regions as an single object,
then use an pointer to point to the region object. The space complexity of {\bf Direct} index is $O(N^2)$,
and the time complexity of query is $O(1)$.
