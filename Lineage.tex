...
%A diagnostic tool that allows easy integration with the ML framework, minimal performance penalty, quick and accurate inspection of the element mapping
%will illustrate computation insights for users to trace suspicious anomalies, locate bugs in the ML pipeline, and tune the ML model. 
%Fine-grained data lineage at data structure element-level (e.g. elements of a matrix) -- the mapping between
%input elements to output elements and according computation -- can provide useful information for
%such diagnosis~\cite{widom04}. 
...
%The diagnostic system should be easy to use, able to collect lineage information of ML pipelines with a minimal performance penalty, and
...
The weak inversion method~\cite{woodruf97} sacrifices lineage accuracy assuming the queries can tolerate errors.
