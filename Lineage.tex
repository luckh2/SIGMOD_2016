...
Snapshot collects lineage at the transformer level by recording the computation itself, the mapping
between input and output elements, and optionally the input/output dataset along with a placeholder
for random factors. 
Snapshot stores the lineage data on the underneath file system, and builds index over the lineage data
...
native solution incurs unacceptable memory consumption, disk storage. 
...
respectively.
We also present two use case studies of Snapshot for image classification and astronomy object detection.
\end{abstract}
