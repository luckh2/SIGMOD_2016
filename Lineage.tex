...
The measurement shows a performance penalty range of 25.6\%-62.2\%, this performance penalty is comparable to that
...
Then we isolate the SystemX's overhead from the pipelines and show the relative scalability
 to the performance of 8 machines, shown as Figure~\ref{fig:scalability}.

\begin{figure*}[t]
\begin{center}
    \includegraphics[width=120mm]{pictures/Scalability}
\caption {Relative Speedup of the VOCSIFTFisher, Source Extractor, and MNIST Pipelines with SystemX over Eight Nodes.
Speedup is measured by isolating SystemX's overhead.
