...
(data structure cell level, e.g. elements in a matrix). We assume an programming model similar to KeystoneML that builds ML pipelines
as a chain of data transformations, with each transformation having ML semantics.
...
Trio~\cite{widom04} tracks lineage for the RDBMS context with eager materialization of cell mappings.
Recent work in fine-grained lineage capturing such as GMRW~\cite{ikeda11} and Newt~\cite{logothetis13} presents
solutions for mapreduce systems. Both systems expose lineage capturing interface at cell level and construct the 
...
that operates on the underlying system operators (e.g. mapper and combiner in mapreduce systems, UDF in SciDB). 
This approach is general for all workloads running on the system, but it gives the user very limited flexibility for customized
instrumentation (specification of lineage type and the data to be collected).
...
(Figure~\ref{fig:conceptual} shows an example), we are able to identify seven primitive lineage types.
...
and {\bf join} mapping. 
This interface covers 87\% of the data transformations in the current code base of KeystoneML. 
