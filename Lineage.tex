...
resulting in collapse mapping(\textcircled{1}) and expansion mapping(\textcircled{5}, \textcircled{6}). 
The mapping between spaces with the same number of dimensions can be identity mapping, all mapping(\textcircled{8}), 
geometry mapping(\textcircled{2}, \textcircled{4}), and linear-combination mapping(\textcircled{3}, \textcircled{7}).
...
\begin{center}
    ...
    \textcircled{1}: converts multi-channel image to gray scale.
...
By examining the KeystoneML code base, we see that Gineala's lineage interface covers x\% of the existing transformers.
...
\item{} We present three real use case of using fine-grained data lineage for data, code, and results debugging.
\end{shortlist}
