...
\begin{abstract}
Distributed machine learning (ML) systems are increasingly popular as practitioners and researchers 
can quickly build productive applications (ML pipelines) and execute them at scale. 
However, debugging such a pipeline is often cumbersome due to the lack of system support for
...
identify a suite of primitive lineage types based on its dataflow and cross-space mapping patterns, 
...
We present the Gineala lineage system as part of the KeystoneML, which is a distributed machine learning
...
Machine learning systems are increasingly popular as practitioners and researchers can quickly
...
corrupted training data point and retrain the model.
...
use cases. These solutions are often use case specific, performance degradation prone, and can introduce high code
complexity due to the demand for multiple versions of an ML pipeline.
...
data point from the beginning step forward until the step before model training to figure out the affected features.
Then the user traces the affected features backward and confirms these affected features only depend on the corrupted
data point. At this point, the users can remove the affected features directly from the input of the model training step, 
...
In this paper, we explore the debuggability of distributed machine learning systems by leveraging the fine-grained data lineage.
We formulate the data transformation in the ML pipeline as a multi-dimensional space transformation. 
As shown in Figure~\ref{fig:conceptual}, the mapping of an ML pipeline can be viewed as a sequence of multi-dimentional space transformations. 
Higher-dimensional space can collapse/expand to lower-dimentional space along one dimension at a time, 
resulting in collapse mapping and expansion mapping. 
The mapping between spaces with the same dimensions can be one-to-one mapping, all-to-one mapping, 
region mapping, sample mapping, and linear-combination mapping.
Thus, we define seven lineage primitive types based on the cell-level mapping. 
Users can declare the according lineage type for individual data transformations.
Specifically for region mapping, we introduce an high-order function approach to describe the geometry for
less storage consumption and investigate various indexing strategies of the geometry mapping for higher query performance.

%We present the Gineala lineage system, which is built upon the KeystoneML distributed machine learning system. 
