...
Then we present the scalability of SystemX's lineage capturing with two real ML pipelines on varying cluster sizes.
...
\begin{center}
    ...
    \hline
    Dataset & VOC & SDSS DR7 \\ \hline \hline
    Size & 828~MB & 65~GB \\ \hline
    File Count & 10,000 & 11,150 \\ \hline
    \end{tabular}
    \end{scriptsize}
    \label{tb:datasets}
\end{center}   
...
in 2D space. The total number of mappings has 300 million geometries. 
This test workload is derived from the SIFTExtractor transformer in the SIFTFisher pipeline.
The experiment was executed on a 16 machine cluster.
...
Capturing 300 million geometry mappings takes 89.8~seconds overhead in average. 
Building the index for these mappings takes 33.9, 69.9, and 349.0~seconds for RTree, KMeans-1, and KMeans-5.
...
We evaluate lineage capturing performance with the SIFTFisher and SourceExtractor pipelines. 
...
Figure~\ref{fig:VOC-overhead} shows the isolated wall clock time of lineage capturing performance for the meta-only and meta+data
...
to 16 machines. Then the performance starts to slow down with more machines due to the small input file size and short task runtime.
...
One pattern we can conclude from the scalability measurements is that SystemX's lineage capturing performance scales no worse than the ML pipelines.
This is due to the fact that SystemX captures and stores the lineage in a distributed manner with the RDD data structure. 
...
on a transformer can return a large number of coordinates, e.g., on the order of 1M. 
