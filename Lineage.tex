...
The model is required if the transformer is an output of an estimator.
...
Right: Three element-wise mapping cases.
...
As can be seen in the region mapping example in Figure~\ref{fig:narrowmapping}, the backward query of an output element should return all the elements in the mapped region. If the output region has $N$ elements and the input region has $M$ elements, the space complexity of storing such a mapping is $O(N*M).$

\subsection{Lineage Type}
Gineala defines two lineage interfaces at high level: NarrowLineage and GatherLineage. 
NarrowLineage covers the lineage that has a narrow dependency property and GatherLineage covers the dependency in a gather pattern, where the transformer takes a sequence of RDD of data structures and transform them into a single RDD of data structures. 

The NarrowLineage can then be further specified in a naming convention of mapping pattern. E.g., NarrowLineage that has One Mapping is named as OneLineage.
...
For RegionLineage and SampleLineage, Gineala requires the mapping collection for the declaration. Such a collection should be a RDD. 
Each RDD element is a list of tuples.
